...
http_callback_404(httpd *webserver, request *r)
{
	...
	if (!is_online()) {
		...
		if (!(mac = arp_get(r->clientAddr))) {
			...
			safe_asprintf(&urlFragment, "%sdev_id=%s&gw_address=%s&gw_port=%d&gw_id=%s&ip=%s&url=%s",
			...
			safe_asprintf(&urlFragment, "%sdev_id=%s&gw_address=%s&gw_port=%d&gw_id=%s&ip=%s&client_mac=%s&url=%s",
				auth_server->authserv_login_script_path_fragment,
                config->device_id,
				config->gw_address,
				config->gw_port, 
...
{
	send_http_page(r, "SmartWiFi", "Please use the menu to navigate the features of this SmartWiFi access point.");
}

void 
http_callback_about(httpd *webserver, request *r)
{
	send_http_page(r, "About SmartWiFi", "This is SmartWiFi version <strong>" VERSION "</strong>");
}
...
http_callback_status(httpd *webserver, request *r)
{
	...
	send_http_page(r, "SmartWiFi Status", buf);
	free(buf);
	free(status);
}
...
void http_send_redirect_to_portal(request *r, const char *urlFragment, const char *text)
{
	char *protocol = NULL;
	int port = 80;
    t_portal_serv *portal_server = get_portal_server();

	if (portal_server->portalserv_use_ssl) {
		protocol = "https";
		port = portal_server->portalserv_ssl_port;
	} else {
		protocol = "http";
		port = portal_server->portalserv_http_port;
	}
			    		
	char *url = NULL;
	safe_asprintf(&url, "%s://%s:%d%s%s",
		protocol,
		portal_server->portalserv_hostname,
		port,
		portal_server->portalserv_path,
		urlFragment
	);
	http_send_redirect(r, url, text);
	free(url);	
...
http_callback_auth(httpd *webserver, request *r)
{
	...
	if ((token = httpdGetVariableByName(r, "token"))) {
		...
		if (!(mac = arp_get(r->clientAddr))) {
			...
			send_http_page(r, "SmartWiFi Error", "Failed to retrieve your MAC address");
		} else {
		...
		send_http_page(r, "SmartWiFi error", "Invalid token");
	}
}

void http_callback_active(httpd *webserver, request *r)
{
    httpVar *dev_id;
    s_config	*config = config_get_config();
	t_authresponse	auth_response;

    dev_id = httpdGetVariableByName(r, "dev_id");
    if(dev_id) {
        auth_server_register(&auth_response, dev_id->value, config->gw_id);
        if(auth_response.authcode == AUTH_ALLOWED) {
            send_http_page(r, "SmartWiFi Register", "Success!");
        } else {
            send_http_page(r, "SmartWiFi Register", "Not completed!");
        }
    } else {
        debug(LOG_ERR, "smartwifi active no dev_id");
        send_http_page(r, "SmartWiFi Register", "Fail! No device id.");
    }
}
