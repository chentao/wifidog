...
void iptables_fw_clear_whiteurls(void)
{
	iptables_do_command("-t filter -F " TABLE_SMART_WHITEURLS);
	iptables_do_command("-t nat -F " TABLE_SMART_WHITEURLS);
}

void iptables_fw_clear_trustedmacs(void)
{
    iptables_do_command("-t mangle -F " TABLE_WIFIDOG_TRUSTED);
}

void iptables_fw_set_trustedmacs(const char *params)
{
    cJSON *root = NULL;
    cJSON *arr = NULL;
    cJSON *item = NULL;
    ...
    if(NULL == (root = cJSON_Parse(params))) {
        debug(LOG_ERR, "Fail parse json params: %s", params);
        return;
    }
    if(NULL == (arr = cJSON_GetObjectItem(root, "mac")) || arr->type != cJSON_Array) {
        cJSON_Delete(root);
        debug(LOG_ERR, "Params mac array wrong.");
        return;
    }
    for(i = 0; i < cJSON_GetArraySize(arr); i++) {
        if(NULL != (item = cJSON_GetArrayItem(arr, i))) {
            mac = safe_strdup(item->valuestring);
            iptables_do_command("-t mangle -A " TABLE_WIFIDOG_TRUSTED " -m mac --mac-source %s -j MARK --set-mark %d", mac, FW_MARK_KNOWN);
            free(mac);
        }
    }
    cJSON_Delete(root);
...
iptables_fw_set_authservers(void)
{
    ...
    t_platform_serv *platformserv;
    t_portal_serv *portalserv;
	...
	for (platformserv = config->platform_servers; platformserv != NULL; platformserv = platformserv->next) {
		if (platformserv->last_ip && strcmp(platformserv->last_ip, "0.0.0.0") != 0) {
			iptables_do_command("-t filter -A " TABLE_WIFIDOG_AUTHSERVERS " -d %s -j ACCEPT", platformserv->last_ip);
			iptables_do_command("-t nat -A " TABLE_WIFIDOG_AUTHSERVERS " -d %s -j ACCEPT", platformserv->last_ip);
		}
	}

	for (portalserv = config->portal_servers; portalserv != NULL; portalserv = portalserv->next) {
		if (portalserv->last_ip && strcmp(portalserv->last_ip, "0.0.0.0") != 0) {
			iptables_do_command("-t filter -A " TABLE_WIFIDOG_AUTHSERVERS " -d %s -j ACCEPT", portalserv->last_ip);
			iptables_do_command("-t nat -A " TABLE_WIFIDOG_AUTHSERVERS " -d %s -j ACCEPT", portalserv->last_ip);
		}
	}
}

/*TODO: chent*/
void iptables_fw_set_whiteurls(void)
{
    const s_config *config;
    t_white_url *white_url;

    config = config_get_config();

    for(white_url = config->white_urls; white_url != NULL; white_url = white_url->next) {
...
void enter_escape_mode(void)
{
	...
	iptables_do_command("-t filter -D " TABLE_WIFIDOG_UNKNOWN " -j REJECT --reject-with icmp-port-unreachable");
	iptables_do_command("-t nat -D " TABLE_WIFIDOG_UNKNOWN " -p tcp --dport 80 -j REDIRECT --to-ports %d", gw_port);
}

void leave_escape_mode(void)
{
	int gw_port = 0;
	const s_config *config;
	config = config_get_config();
