...
#endif	
#define SMARTWIFI_CONFIGFILE "/etc/smartwifi.json"
#define SMARTWIFI_URLS_CONFIGFILE "/etc/smartwifi_urls.json"
#define DEFAULT_DAEMON 1
#define DEFAULT_DEBUGLEVEL LOG_DEBUG
...
typedef struct _white_url_t {
    char *url;
    time_t last_update;
    struct _white_url_t *next;
} t_white_url;

typedef struct _platform_serv_t {
    char *platformserv_hostname;
    char *platformserv_path;
    int platformserv_http_port;
    int platformserv_ssl_port;
    int platformserv_use_ssl;
    char *last_ip;
    struct _platform_serv_t *next;
} t_platform_serv;

typedef struct _portal_serv_t {
    char *portalserv_hostname;
    char *portalserv_path;
    int portalserv_http_port;
    int portalserv_ssl_port;
    int portalserv_use_ssl;
    char *last_ip;
    struct _portal_serv_t *next;
} t_portal_serv;
...
typedef struct {
    ...
    char *account;
    char *active_date;
    t_platform_serv *platform_servers;
    t_portal_serv *portal_servers;
    t_white_url *white_urls;
} s_config;
