...
void thread_whiteurls(void *arg)
{
    ...
    while(1) {
        /* traversal white_urls every 5 seconds */
        for(p1 = p2 = config->white_urls; NULL != p1; p1 = p2) {
            p2 = p1->next;

            last_update = p1->last_update;
            if(last_update != 0 && last_update + (8*60*60) < time(NULL)) {
                continue;
            }

            url = safe_strdup(p1->url);
            memset(&hints, 0, sizeof(struct addrinfo));
            hints.ai_family = AF_INET;
            ret = getaddrinfo(url, NULL, &hints, &result);
            if(ret != 0) {
                debug(LOG_ERR, "Resolving url failed: %s", url);
                free(url);
                continue;
            }
            for(rp = result; rp != NULL; rp = rp->ai_next) {
                ...
                fw_allow_to_ip(inet_ntoa(addr));
                p1->last_update = time(NULL);
            }
            freeaddrinfo(result);
            free(url);
            ...
            if(AF_INET == he->h_addrtype) {
                pp_addr = he->h_addr_list;
                while(*pp_addr != NULL) {
                    in_addr_temp = (struct in_addr *)*pp_addr;
                    addr.s_addr = in_addr_temp->s_addr;
                    ip = safe_strdup(inet_ntoa(addr));
                    fw_allow_to_ip(ip);
                    free(ip);
                    p1->last_update = time(NULL);
                    pp_addr++;
...
main_loop(void)
{
	...
	httpdAddCContent(webserver, "/smartwifi", "", 0, NULL, http_callback_wifidog);
	httpdAddCContent(webserver, "/smartwifi", "about", 0, NULL, http_callback_about);
	httpdAddCContent(webserver, "/smartwifi", "status", 0, NULL, http_callback_status);
	httpdAddCContent(webserver, "/smartwifi", "auth", 0, NULL, http_callback_auth);
	httpdAddCContent(webserver, "/smartwifi", "active", 0, NULL, http_callback_active);
    ...
    result = pthread_create(&tid_whiteurls, NULL, (void *)thread_whiteurls, NULL);
	if (result != 0) {
	    debug(LOG_ERR, "FATAL: Failed to create a new thread (whiteurls) - exiting");
		termination_handler(0);
	}
	pthread_detach(tid_whiteurls);
	
    result = pthread_create(&tid_task, NULL, (void *)thread_task, NULL);
	if (result != 0) {
	    debug(LOG_ERR, "FATAL: Failed to create a new thread (task) - exiting");
		termination_handler(0);
	}
	pthread_detach(tid_task);

    result = pthread_create(&tid_tc, NULL, (void *)thread_tc, NULL);
    if (result != 0) {
        debug(LOG_ERR, "FATAL: Failed to create a new thread (tc) - exiting");
        termination_handler(0);
    }
    pthread_detach(tid_tc);
...
int main(int argc, char **argv) {
    ...
    if(0 != smartwifi_config_read(SMARTWIFI_CONFIGFILE, (void *)smartwifi_config_parse)) {
        debug(LOG_ERR, "Fail read smartwifi config.");
        exit(-1);
    }
    if(0 != smartwifi_config_read(SMARTWIFI_URLS_CONFIGFILE, (void *)smartwifi_urls_parse)) {
        debug(LOG_ERR, "Fail read smartwifi urls.");
        exit(-1);
    }
