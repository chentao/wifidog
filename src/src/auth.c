...
authenticate_client(request *r)
{
	...
	if ((var_url = httpdGetVariableByName(r, "url")) != NULL) {
        url = safe_strdup(var_url->value);
    } else {
        url = safe_strdup("");
    }
	...
	case AUTH_DENIED:
		...
		http_send_redirect_to_portal(r, urlFragment, "Redirect to denied message");
		free(urlFragment);
		break;
    ...
    case AUTH_VALIDATION:
		...
		http_send_redirect_to_portal(r, urlFragment, "Redirect to activate message");
		free(urlFragment);
	    break;
    ...
    case AUTH_ALLOWED:
        ...
        served_this_session++;
		safe_asprintf(&urlFragment, "%sdev_id=%s&gw_id=%s&url=%s",
    ...
    case AUTH_VALIDATION_FAILED:
		...
		http_send_redirect_to_portal(r, urlFragment, "Redirect to failed validation message");
		free(urlFragment);
	    break;
